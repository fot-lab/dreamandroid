// LCMSampler implementation
// Based on HuggingFace diffusers LCMScheduler (scheduling_lcm.py)
#ifndef LCMSAMPLER_HPP
#define LCMSAMPLER_HPP
...
class LCMSampler : public Sampler {
 public:
  LCMSampler(int num_train_timesteps, float beta_start, float beta_end,
  ...
  SamplerOutput step(const xt::xarray<float> &model_output, int timestep,
    ...
    if (prediction_type_ == "epsilon") {
      ...
      throw std::runtime_error(prediction_type_ +
                               " is not implemented for LCMSampler");
    }
