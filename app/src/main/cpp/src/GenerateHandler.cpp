...
GenerationResult generateImage(
  ...
  try {
    ...
    for (int i = start_step; i < timesteps.size(); ++i) {
      if (req.show_diffusion_process && !conf.use_mnn && !sdxl_lowram &&
        ...
        try {
          ...
          if ((cur_out_w > 512 || cur_out_h > 512) && !conf.sdxl_mode) {
            ...
            if (tile_success) {
              pixels = blendVaeOutputTiles(
                  decoded_tiles, output_positions, cur_out_h, cur_out_w,
            ...
            StatusCode vae_dec_status =
                conf.sdxl_mode
                    ...
                    : models.vaeDecoderApp->executeVaeDecoderGraphs(
                          vae_dec_in_vec.data(), vae_dec_out_pixels.data());
            if (StatusCode::SUCCESS == vae_dec_status) {
              std::vector<int> pixel_shape = {1, 3, cur_out_h,
                                              cur_out_w};
          ...
          if (preview_success) {
            ...
            if (req.aspect_pad_inpaint && req.target_crop_width > 0 &&
              ...
              for (int y = 0; y < req.target_crop_height; ++y) {
                const uint8_t *src_row =
                    out_data.data() +
                    ((size_t)(py0 + y) * cur_out_w + px0) * 3;
                uint8_t *dst_row =
                    cropped.data() + (size_t)y * req.target_crop_width * 3;
