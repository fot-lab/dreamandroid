// self-implemented DPMSolverMultistepSampler class
#include <cmath>
#include <optional>
#include <string>
#include <vector>
...
class DPMSolverMultistepSampler : public Sampler {
 public:
  DPMSolverMultistepSampler(int num_train_timesteps, float beta_start,
  ...
  xt::xarray<float> convert_model_output(const xt::xarray<float> &model_output,
    ...
    if (prediction_type_ == "epsilon") {
      ...
      throw std::runtime_error(
          prediction_type_ +
          " is not implemented for DPMSolverMultistepSampler");
    }
  }
  ...
  SamplerOutput step(const xt::xarray<float> &model_output, int timestep,
